...
if ( ! function_exists( 'landx_entry_header_meta' ) ) :
/**
 * Set up post entry meta.
 *
 * Prints HTML with meta information for current post: categories, tags, permalink, author, and date.
 *
 * Create your own landx_entry_meta() to override in a child theme.
 *
 */
function landx_entry_header_meta() {

	printf( '<a class="blog_icons author vcard url fn n" href="%1$s" title="%2$s" rel="author"><i class="fa fa-user"></i> By %3$s</a>',
		esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
		esc_attr( __( '%s', THEMENAME ), get_the_author()  ),
		get_the_author()
	);

	if(is_single()){
		$categories_list = get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'landx' ) );
		if ( $categories_list ) {
			printf( '<span class="cat-links blog_icons"><span class="screen-reader-text">%1$s </span>%2$s</span>',
				_x( '<i class="fa fa-file-text"></i>', 'Used before category names.', THEMENAME ),
				$categories_list
			);
		}		
	}
...
function landx_one_page_menu(){
	$html = '';
	$html .='<ul class="nav navbar-nav navbar-right main-navigation" id="header-menu-1">
			 <li class="menu-item menu-item-type-post_type menu-item-object-page menu-item-1 current-menu-item">
			 <a href="' .esc_url( get_permalink() ). '" target="_blank">One page menu settings</a></li>
             </ul>';
  echo $html;
}

function landx_onepage_sections_array( $page_id = 0 ){
	$sections = array();
...
class Landx_Onepage_walker_nav_menu extends Walker_Nav_Menu {  
	...
	// add main/sub classes to li's and links
	 function start_el(  &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
	    ...
	    $sections = landx_onepage_sections_array(get_the_id());
	    if(!in_array($item->object_id, $sections)){
		    $attributes .= ! empty( $item->url )        ? ' href="'   . esc_attr( $item->url        ) .'"' : '';
		}else{
		    $attributes .= ! empty( $item->url )        ? ' href="#'   . esc_attr( get_the_slug($item->object_id) ) .'"' : '';
		}
