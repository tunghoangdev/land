...
if( !empty($pages) ):
	...
	foreach ($pages as $key => $value):
		...
		if( ($value['page_id'] != '') && ( $value['link_type'] == 'internal' ) ):
			...
			?>
			<section id="<?php echo get_the_slug($value['page_id']); ?>" class="onepage-<?php echo $pageid; ?> section<?php echo $value['page_id'].$alt; ?> <?php echo ( ($bg_style == 'dark') || ($bg_style == 'color') )? 'cta-section '.$bg_style : 'default-cta';  ?>" 
				<?php echo ( ($bg_style == 'dark') || ($bg_style == 'color') )? 'data-parallax="scroll" data-image-src="'.esc_url(get_post_meta( get_the_ID(), 'background', true )).'"' : ''; ?>>
