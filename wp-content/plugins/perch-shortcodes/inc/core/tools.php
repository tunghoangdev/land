 ...
 */
function perch_hex_shift( $supplied_hex, $shift_method, $percentage = 50 ) {
...
function perch_hex2rgb( $colour, $delimiter = '-' ) {
	if ( $colour[0] == '#' ) {
 ...
 */
function perch_apply_formatting() {
	// Enable shortcodes in text widgets
	add_filter( 'widget_text', 'do_shortcode' );
	// Enable shortcodes in category descriptions
	add_filter( 'category_description', 'do_shortcode' );
	// Enable custom formatting
	if ( get_option( 'perch_option_custom-formatting' ) === 'on' ) {
		// Apply custom formatter function
		add_filter( 'the_content', 'perch_clean_shortcodes' );
	}
}

add_action( 'init', 'perch_apply_formatting' );
 ...
 */
function perch_clean_shortcodes( $content ) {
	$p = perch_cmpt();
	$array = array (
		'<p>[' => '[',
		']</p>' => ']',
 ...
 */
function perch_do_shortcode( $content, $pre ) {
 ...
 */
function perch_compatibility_mode_prefix() {
	return get_option( 'perch_option_prefix' );
}

/**
 * Shortcut for perch_compatibility_mode_prefix()
 */
function perch_cmpt() {
	return perch_compatibility_mode_prefix();
}
 ...
 * @return bool
 */
function perch_addon_active( $addons ) {
	// Prepare add-ons paths
	$paths = array(
		'maker' => 'perch-shortcodes-maker/perch-shortcodes-maker.php',
		'skins' => 'perch-shortcodes-skins/perch-shortcodes-skins.php',
		'extra' => 'perch-shortcodes-extra/perch-shortcodes-extra.php',
	);
...
function perch_skins_link() {
	if ( perch_addon_active( 'skins' ) ) return sprintf( '<br><strong>%s</strong><br><strong>%s</strong>', __( 'Additional skins successfully installed', 'perch' ), __( 'Open dropdown to choose one of new styles', 'perch' ) );
	else return sprintf( '<br><a href="http://gndev.info/sus" target="_blank">%s &rarr;</a>', __( 'Get more styles', 'perch' ) );
}
 ...
 */
function perch_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
	global $wp_version;
	...
	if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
		...
		if ( !file_exists( $dest_file_name ) ) {
			...
			if( function_exists('retina_support_create_images') ){
				retina_support_create_images( get_attached_file( $get_attachment[0]->ID ), $width, $height, true );
			}			
 ...
 */
function perch_delete_resized_images( $post_id ) {
...
add_action( 'delete_attachment', 'perch_delete_resized_images' );

class Tp_Tools {
	function __construct() {
		add_action( 'wp_ajax_perch_example_preview', array( __CLASS__, 'example' ) );
		add_action( 'perch/update',                  array( __CLASS__, 'reset_examples' ) );
		add_action( 'perch/activation',              array( __CLASS__, 'reset_examples' ) );
	...
	public static function get_posts_dropdown( $args = array() ) {
		global $wpdb, $post;
	...
	public static function get_slides( $args ) {
		...
		if ( $args['gallery'] !== null || ( $args['source'] === 'none' && get_option( 'perch_option_galleries-432' ) ) ) return self::get_slides_432( $args );
		...
		if ( is_array( $query->posts ) ) foreach ( $query->posts as $post ) {
				...
				elseif ( $args['link'] === 'custom' ) $slide['link'] = get_post_meta( $post->ID, 'perch_slide_link', true );
	...
	public static function get_slides_432( $args ) {
		...
		$galleries = get_option( 'perch_option_galleries-432' );
		// No galleries found
	...
	public static function example() {
		...
		self::access();
		// Check nonce
		if ( !isset( $_REQUEST['nonce'] ) || !wp_verify_nonce( $_REQUEST['nonce'], 'perch_examples_nonce' ) ) return;
		// Check incoming data
		if ( !isset( $_REQUEST['code'] ) || !isset( $_REQUEST['id'] ) ) return;
		// Check for cache
		$output = get_transient( 'perch/examples/render/' . sanitize_key( $_REQUEST['id'] ) );
		...
		// Cache not found
			...
			// Check for code
			if ( !$code ) die( '<p class="perch-examples-error">' . __( 'Example code does not found, please check it later', 'perch' ) . '</p>' );
			// Clean-up the code
			$code = str_replace( array( "\t", '%perch_' ), array( '  ', perch_cmpt() ), $code );
			// Split code
			$chunks = explode( '-----', $code );
			// Show snippets
			do_action( 'perch/examples/preview/before' );
			foreach ( $chunks as $chunk ) {
				...
				echo '<div class="perch-examples-code"><span class="perch-examples-get-code button"><i class="fa fa-code"></i>&nbsp;&nbsp;' . __( 'Get the code', 'perch' ) . '</span><textarea rows="' . $rows . '">' . esc_textarea( $chunk ) . '</textarea></div>';
			}
			do_action( 'perch/examples/preview/after' );
			$output = ob_get_contents();
			ob_end_clean();
			set_transient( 'perch/examples/render/' . sanitize_key( $_REQUEST['id'] ), $output );
			echo $output;
		}
		die();
	}
	...
	public static function reset_examples() {
		foreach ( (array) Tp_Data::examples() as $example ) foreach ( (array) $example['items'] as $item ) delete_transient( 'perch/examples/render/' . $item['id'] );
	}
	...
	public static function get_icon( $args ) {
		...
		if ( strpos( $args['icon'], 'icon:' ) !== false ) {
			...
			perch_query_asset( 'css', 'ionicons' );
			// Return icon
	...
	public static function icons() {
		$icons = array();
		$output = '';
		$list = '<ul class="iconset-list">';
		if ( is_callable( array( 'Tp_Data', 'icons' ) ) ) foreach ( (array) Tp_Data::icons() as $key => $iconset ) {
				
				$list .= '<li data-target=".'.$key.'">'.$key.'</li>';
				$output .= '<div class="iconset '.$key.'" style="display:none">';
				$icons = array();
				foreach ($iconset as $icon) {
					$icons[] = '<i class="' . $icon . '" title="' . $icon . '"></i>';
				}
				$output .= implode( '', $icons );
				$output .= '</div>';
			}
		$list .= '</ul>';	
		return $list.$output;
	...
	public static function slide_link_input( $form_fields, $post ) {
		$form_fields['perch_slide_link'] = array(
			'label' => __( 'Slide link', 'perch' ),
			'input' => 'text',
			'value' => get_post_meta( $post->ID, 'perch_slide_link', true ),
			'helps' => sprintf( '<strong>%s</strong><br>%s', __( 'Perch Shortcodes', 'perch' ), __( 'Use this field to add custom links to slides used with Slider, Carousel and Custom Gallery shortcodes', 'perch' ) )
		);
		return $form_fields;
	}

	public static function slide_link_save( $post, $attachment ) {
		if ( isset( $attachment['perch_slide_link'] ) )
			update_post_meta( $post['ID'], 'perch_slide_link', $attachment['perch_slide_link'] );
		return $post;
	}

	public static function error( $prefix = false, $message = false ) {
		if ( !$prefix && !$message ) return '';
		$return = array( '<div class="perch-error" style="padding:10px;border:1px solid #f03;color:#903;background:#fde">' );
...
/**
 * Shortcut for Tp_Tools::decode_shortcode()
 */
function perch_scattr( $value ) {
	return Tp_Tools::do_attr( $value );
}

/**
 * Shortcut for Tp_Tools::get_icon()
 */
function perch_get_icon( $args ) {
	return Tp_Tools::get_icon( $args );
}
